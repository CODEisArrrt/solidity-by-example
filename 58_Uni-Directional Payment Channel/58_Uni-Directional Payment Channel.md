...
contract UniDirectionalPaymentChannel is ReentrancyGuard {
    ...
    //在部署合约时，构造函数将指定接收方地址，并设置通道的过期时间。
    ...
    //私有函数，用于生成将要签名的哈希值。它将此合约地址和发送的金额作为输入，并返回哈希值。
    ...
    //这个函数是 _getHash() 的变体，它将哈希值转换为以太币签名哈希值。这是为了防止重入攻击。
    ...
    //用于验证签名。它将发送的金额和签名作为输入，并使用 recover() 函数从签名中恢复签名者地址。如果地址与发送方地址匹配，则返回 true。
    ...
    //函数用于关闭通道并将指定金额发送给接收方。它需要接收方提供签名来验证交易，然后将指定金额发送给接收方，并销毁合约。
